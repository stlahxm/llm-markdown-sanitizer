...
- **Codecov** — coverage is uploaded from Python's `test (3.12)` matrix leg only (not all four) and from the Java `test` job, tagged `flags: python` / `flags: java`. If you add a new Python matrix version or rename the Java job, update the `if:` condition in `python-ci.yml` accordingly or the upload silently stops.
